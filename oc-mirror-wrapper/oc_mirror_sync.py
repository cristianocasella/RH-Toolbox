    ...
    def _validate_config(self):
        ...
        for idx, registry in enumerate(self.config["registries"]):
            ...
            if "name" not in registry:
                print(
                    f"WARNING: Registry at index {idx} missing 'name' field, using 'Registry-{idx}'"
                )
                registry["name"] = f"Registry-{idx}"

        if not os.path.exists(self.config["imageset_config"]):
            print(
                f"ERROR: ImageSet config file not found: {self.config['imageset_config']}"
            )
            sys.exit(1)
...
class OCMirrorExecutor:
    ...
    def _backup_working_dir(self, stage_name: str):
        ...
        backup_dir = os.path.join(
            self.config.working_folder, f"working-dir-backup-{stage_name}"
        )

        if not os.path.exists(working_dir):
            self.logger.debug("No working-dir to backup for %s", stage_name)
            return
        ...
        try:
            ...
            self.logger.info("Backed up working-dir to %s", backup_dir)
        except (OSError, shutil.Error) as e:
            self.logger.warning("Failed to backup working-dir: %s", e)
    ...
    def _restore_working_dir(self, stage_name: str):
        ...
        if not os.path.exists(backup_dir):
            self.logger.debug("No backup to restore for %s", stage_name)
            return
        ...
        try:
            ...
            self.logger.info("Restored working-dir from %s", backup_dir)
        except (OSError, shutil.Error) as e:
            self.logger.warning("Failed to restore working-dir: %s", e)
    ...
    def _execute_command(
        self, cmd: List[str], stage_name: str
    ) -> Tuple[bool, str, str]:
        """
        ...
        """
        cmd_str = " ".join(cmd)
        self.logger.info("[%s] Executing: %s", stage_name, cmd_str)

        if self.dry_run:
            self.logger.info("[%s] DRY RUN - Command not executed", stage_name)
            return True, "", ""
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                self.logger.info("[%s] Command completed successfully", stage_name)
                return True, output, ""
            print()  # New line after progress completes
            self.logger.error(
                "[%s] Command failed with exit code %s", stage_name, process.returncode
            )
            return False, output, ""

        except FileNotFoundError:
            self.logger.error(
                "[%s] oc-mirror command not found. Ensure oc-mirror is installed and in PATH.",
                stage_name,
            )
            return False, "", "oc-mirror command not found"
        except (OSError, subprocess.SubprocessError) as e:
            self.logger.error("[%s] Unexpected error: %s", stage_name, e)
            return False, "", str(e)
    ...
    def _parse_failed_images(self, working_folder: str) -> List[str]:
        ...
        error_log_pattern = os.path.join(
            working_folder, "working-dir", "logs", "mirroring_errors_*.txt"
        )
        error_logs = glob.glob(error_log_pattern)

        if not error_logs:
            self.logger.warning(
                "No mirroring error logs found at: %s", error_log_pattern
            )
            return failed_images

        # Use the most recent error log
        latest_log = max(error_logs, key=os.path.getmtime)
        self.logger.info("Parsing error log: %s", latest_log)
        ...
        try:
            with open(latest_log, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    match = re.search(
                        r"error mirroring image (?:docker://)?([^\s]+)", line
                    )
                    if match:
                        ...
                        if source_image and source_image not in failed_images:
                            ...
                            self.logger.debug("Found failed image: %s", source_image)
        except (OSError, re.error) as e:
            self.logger.error("Failed to parse error log: %s", e)

        self.logger.info("Parsed %s failed images from error log", len(failed_images))
        return failed_images

    def _retry_with_skopeo(
        self, failed_images: List[str], destination_registry: str
    ) -> Tuple[int, int]:
        """
        ...
        self.logger.info("\n%s", "=" * 80)
        self.logger.info(
            "Retrying %s failed images with skopeo fallback", len(failed_images)
        )
        self.logger.info("%s\n", "=" * 80)
        ...
        for source_image in failed_images:
            ...
            dest_image = dest_image.replace(
                "registry.access.redhat.com", destination_registry
            )

            self.logger.info("Skopeo retry: %s", source_image)
            ...
            try:
                ...
                if result.returncode == 0:
                    self.logger.info("  ✓ Success (direct copy)")
                    successful += 1
                    continue
                if "manifest invalid" in result.stderr:
                    # Attempt 2: OCI format conversion for manifest incompatibility
                    self.logger.info("  → Manifest invalid, trying OCI conversion...")
                    ...
                    try:
                        ...
                        if result_pull.returncode != 0:
                            self.logger.error(
                                "  ✗ OCI pull failed: %s", result_pull.stderr
                            )
                            failed += 1
                            continue
                        ...
                        if result_push.returncode == 0:
                            self.logger.info("  ✓ Success (OCI conversion)")
                            successful += 1
                        else:
                            self.logger.error(
                                "  ✗ OCI push failed: %s", result_push.stderr
                            )
                            failed += 1
                    ...
                    self.logger.error("  ✗ Failed: %s", result.stderr)
                    failed += 1
            except subprocess.TimeoutExpired:
                self.logger.error("  ✗ Timeout after 10 minutes")
                failed += 1
        ...
        self.logger.info("\n%s", "=" * 80)
        self.logger.info(
            "Skopeo fallback results: %s successful, %s failed", successful, failed
        )
        self.logger.info("%s\n", "=" * 80)
    ...
    def mirror_to_registry(self, registry: Dict, registry_index: int) -> bool:
        """
        ...
        success, _, _ = self._execute_command(
            cmd, f"STAGE {stage_num} ({registry_name})"
        )
        ...
        if not success and self.config.skopeo_fallback:
            ...
            if failed_images:
                _, failed_count = self._retry_with_skopeo(
                    failed_images, registry["url"]
                )
...
class WorkflowOrchestrator:
    ...
    def run(self) -> int:
        ...
        self.logger.info("=" * 80)
        self.logger.info("ImageSet Config: %s", self.config.imageset_config)
        self.logger.info("Working Folder: %s", self.config.working_folder)

        # Log all configured registries
        for idx, registry in enumerate(self.config.registries):
            registry_name = registry.get("name", f"Registry-{idx + 1}")
            self.logger.info(
                "Registry %s: %s (%s)", idx + 1, registry_name, registry["url"]
            )

        self.logger.info("V2 Mode: %s", self.config.v2_mode)
        self.logger.info("Stages to run: %s", self.stages)
        self.logger.info("=" * 80)
        ...
        for idx, registry in enumerate(self.config.registries):
            ...
            if stage_num in self.stages:
                ...
                if not self.executor.mirror_to_registry(registry, idx):
                    self.logger.warning(
                        "STAGE %s FAILED - Continuing to next registry", stage_num
                    )
        ...
        return 3 if registry_failed else 0
    ...
    def _print_summary(self, stage_times: Dict[str, float], total_time: float):
        ...
        for stage, success in self.results.items():
            ...
            self.logger.info(
                "%s %s: %s (%s)", symbol, stage.upper(), status, elapsed_str
            )

        self.logger.info("%s", "=" * 80)
        self.logger.info("Total execution time: %s", self._format_time(total_time))
        self.logger.info("%s", "=" * 80)
