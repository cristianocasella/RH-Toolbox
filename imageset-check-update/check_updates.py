...
class RegistryClient:
    ...
    def get_tags(self, registry: str, repository: str) -> List[str]:
    ...
    def find_newer_versions(
        self, registry: str, repository: str, current_tag: str
    ) -> Optional[str]:
        ...
        tags = self.get_tags(registry, repository)
        ...
        current_pattern = re.match(
            r"^v?(\d+)\.?(\d+)?(?:\.(\d+))?(?:\.(\d+))?", current_tag
        )
        ...
        if current_pattern:
            ...
            for tag in tags:
                ...
                if any(
                    skip in tag.lower()
                    for skip in ["sha256", "latest", "source", "temp"]
                ):
                    continue
                ...
                if tag_match:
                    ...
                    if (
                        tag_parts[0] == current_parts[0]
                        and tag_parts[1] == current_parts[1]
                    ):
...
def parse_imageset(file_path: Path) -> Dict:
    ...
    if min_version_match:
        results["openshift_versions"].append(
            {"type": "minVersion", "current": min_version_match.group(1)}
        )

    if max_version_match:
        results["openshift_versions"].append(
            {"type": "maxVersion", "current": max_version_match.group(1)}
        )

    # Extract operator catalogs
    for match in re.finditer(
        r"catalog:\s*(registry\.redhat\.io/redhat/(?:redhat|community)-operator-index):v([0-9]+\.[0-9]+)",
        content,
    ):
        results["operator_catalogs"].append(
            {"catalog": match.group(1), "version": match.group(2)}
        )
    ...
    for match in re.finditer(r"-\s*name:\s*([^:\s]+):([^\s]+)", content):
        ...
        results["additional_images"].append(
            {"full": f"{image_name}:{tag}", "name": image_name, "tag": tag}
        )
...
def check_updates(imageset_path: Path, docker_config: Optional[Path] = None):
    ...
    if results["openshift_versions"]:
        ...
        for ver in results["openshift_versions"]:
            ...
            print("    → Check: https://access.redhat.com/downloads/content/290")
        print()

    # Check operator catalogs
    if results["operator_catalogs"]:
        print("Operator Catalogs:")
        print("-" * 70)
        seen = set()
        for cat in results["operator_catalogs"]:
            ...
            if key not in seen:
                catalog_type = (
                    "Red Hat" if "redhat-operator" in cat["catalog"] else "Community"
                )
...
Examples:
  ...
  %(prog)s imagesets/imageset-config-v2-4.20.21.yaml --docker-config .docker-config.json
    ...
    parser.add_argument(
        "--list", action="store_true", help="List available imageset files and exit"
    )
